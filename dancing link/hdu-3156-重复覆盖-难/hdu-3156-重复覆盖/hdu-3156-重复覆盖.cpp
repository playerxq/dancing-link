...
//向量的旋转 //底边线段ab 绕a逆时针旋转角度A，b->c，sinl是sinA的值。 
        ...
        while(right>=left)
        {
            ...
            for(i=1;i<=n;i++)
            {
                ...
                for(j=1;j<=n;j++)
                    ...
                    if(!hashx[bit]) {hashx[bit]=1;st[++sum]=bit;}
            }
            ...
            {
                for(j=1;j<=n;j++)
                    if(st[i]&dir[j-1]) hashx[st[i]-dir[j-1]]=0;//这里只是标记了一部分子集
                if(hashx[st[i]]==0) continue;
                r++;
                for(j=1;j<=n;j++)
                    if(st[i]&dir[j-1]) {Link(r,j);hashx[dir[j-1]]=0;}//再标记一部分子集
            }
